...
if( !class_exists( 'Aegis' ) ) {
	...
	class Aegis {
		...
		public function add_meta_boxes() {
			add_meta_box( 'aegis_metabox', esc_html__( 'Aegis', 'aegis' ), array( $this, 'get_metabox_form' ), 'page' );
		}
			...
			if ( ! empty( $param_args['type'] ) ) :
				...
				<div class="a_control a_wrap a_clearfix">
					<div class="a_row a_clearfix">
						...
						?>
							<div class="a_col_3"><?php echo esc_attr( $param_args['title'] ); ?></div>
						<?php endif; ?>
		...
		public function get_field_text( $params ) {
			...
			?>
			<input name="<?php echo esc_attr( $params['name'] ); ?>" value="<?php echo esc_attr( $params['value'] ); ?>" type="text" class="a_ui a_ui_text <?php echo esc_attr( $class ); ?>" autocomplete="off">
			<?php
		}

		public function get_field_number( $params ) {
			?>
			<input name="<?php echo esc_attr( $params['name'] ); ?>" value="<?php echo esc_attr( $params['value'] ); ?>" type="text" class="a_ui a_ui_number a_size_20p" autocomplete="off">
			<?php if ( $params['affix'] ) : ?>
		...
		public function get_field_color( $params ) {
			...
			?>
			<input name="<?php echo esc_attr( $params['name'] ); ?>" value="<?php echo esc_attr( $params['value'] ); ?>" type="text" class="a_ui a_ui_color a_size_20p" data-default-color="<?php echo esc_attr( $default ); ?>" autocomplete="off">
			<?php
		}
		...
		public function get_field_checkbox( $params ) {
			...
			?>
			<input <?php checked( $params['value'], 'true' ); ?> name="<?php echo esc_attr( $params['name'] ); ?>" value="true" type="checkbox" class="a_ui_checbox" autocomplete="off">
			<?php
		}
			...
			foreach ( $params['options'] as $value => $title ) :
				...
				<div class="a_clearfix a_checkboxes_wrap <?php echo esc_attr( $classes ); ?>">
					<label title="" for="<?php echo esc_attr( $checkbox_id ); ?>">
						<input <?php echo esc_attr( $checked ); ?> id="<?php echo esc_attr( $checkbox_id ); ?>" name="<?php echo esc_attr( $params['name'] ); ?>[]" value="<?php echo esc_attr( $value ); ?>" type="checkbox" class="a_ui_checbox" autocomplete="off">
						<span><?php echo wp_kses( htmlspecialchars_decode( $title['title'] ), self::get_allowed_tags() ); ?></span>
					</label>
					<span class="a_desc_handler"><?php esc_html_e( '[?]', 'aegis' ); ?></span>
		...
		public function get_field_select( $params ) {
			?>
			<select name="<?php echo esc_attr( $params['name'] ); ?>" class="a_ui a_ui_select a_size_30p" autocomplete=off>
		...
		public function get_field_image( $params ) {
			...
			<div class="a_row a_ui_image">
				...
				<div class="a_col_1">
					<span class="a_size_100p a_image_add button button-secondary"><?php esc_html_e( '+', 'enliven' ); ?></span>
				</div>

				<div class="a_col_1">
					<span class="a_size_100p a_image_remove button button-secondary"><?php esc_html_e( '-', 'enliven' ); ?></span>
				</div>
			</div>
			<?php
		}
		...
		public function get_field_spacing( $params ) {
			...
			<div class="a_ui_spacing">
				<div class="a_first a_row a_clearfix">
					<div class="a_col_2">
						<i><?php esc_html_e( 'Margin', 'aegis' ); ?></i>
					</div>

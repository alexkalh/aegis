...
Aegis plugin, Copyright 2014 Kopatheme.com
...
class Aegis{
	...
	public function get_metabox_form($post){
		...
		<div class="a_wrap a_clearfix">
			...
			<div class="a_header a_clearfix">
				<span class="a_add_row a_pull_left button button-secondary"><?php _e('Add row', 'aegis'); ?></span>
				<span class="a_save_all a_pull_right button button-primary"><?php _e('Save all', 'aegis'); ?></span>
			</div>
  ...
  public function get_field_select($params){
  	?>
		<select 
			name="<?php echo esc_attr($params['name']); ?>" 
			class="a_ui a_ui_select a_size_50p"
			autocomplete=off>
  ...
  public function esc_data($param_args, $value){
		switch ($param_args['type']) {
			...
			case 'image':
				if (!empty($value)) {
          $value = str_replace(home_url(), '[aegis_home_url]', $value);
      	}
				break;
			case 'select':
				$value = esc_attr($value);
